...
#ifdef WIN32
				char txt[256];
				sprintf(txt,"%s%s","[OV Unicode UTF16LE Hex] U+",keyseq.buf);
				srv->notify(txt);
#else
				textbar->clear()->append((char *)"[OV UnicodeHex] U+")->append(keyseq.buf)->update()->show();
#endif
			}
...
public:
    ...
    virtual const char* localizedName(const char *locale) {
        if (!strcasecmp(locale, "zh_TW") || !strcasecmp(locale, "zh-Hant"))
            return "Unicode \xE5\x85\xA7\xE7\xA2\xBC"; //內碼
        else if (!strcasecmp(locale, "zh_CN")|| !strcasecmp(locale, "zh-Hans"))
            return "Unicode \xE5\x86\x85\xE7\xA0\x81"; //内码
