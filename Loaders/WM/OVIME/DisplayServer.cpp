...
AVDisplayServer *DisplayServer::selectNextCandItem(int i)
{
	...
	switch (i) {
		case 1: //line down
			{
				if(lpCandList->dwSelection < lpCandList->dwCount-1 ) lpCandList->dwSelection++;
				if(lpCandList->dwSelection > (lpCandList->dwPageStart + lpCandList->dwPageSize-1)){
					lpCandList->dwPageStart += lpCandList->dwPageSize;
				DWORD dwMax = lpCandList->dwCount - lpCandList->dwPageSize;
		...
		case -1: //line up
			{
			 if (lpCandList->dwSelection > 0) lpCandList->dwSelection--;
			 if (lpCandList->dwSelection < lpCandList->dwPageStart) lpCandList->dwPageStart -= lpCandList->dwPageStart;		
        	}	
			break;
		case 2: // Page down
