	...
	case WM_IME_SETCONTEXT:  //switch between windows
		...
		switch(lParam)
		{
			case ISC_SHOWUICANDIDATEWINDOW :  
				murmur("\tISC_SHOW//UICANDIDATEWINDOW ");
				break; 
			case ISC_SHOWUICOMPOSITIONWINDOW :
				murmur("\tISC_SHOW//UICOMPOSITIONWINDOW  ");
				break; 
			case ISC_SHOWUIGUIDELINE :
				murmur("\tISC_SHOW//UIG//UIDELINE ");
				break;
			case ISC_SHOWUIALLCANDIDATEWINDOW :
				murmur("\tISC_SHOW//UIALLCANDIDATEWINDOW");
				break;
			case ISC_SHOWUIALL :
				murmur("\tISC_SHOW//UIALL");
				break;
			default: 
...
//				murmur("IM module id #%d", moduleId);
				...
				//if(lParam & ISC_SHOW//UICOMPOSITIONWINDOW) // not sure
				//{	
					//dsvr->showBuf(true);
				//}
	...
	case WM_IME_SELECT:
		...
		if(wParam) //the indicated IME is selected
		{
			...
			{ 
				...
				AVLoader* loader = AVLoader::open();
				loader->connectDisplayServer(dsvr);
				//if(loader->GetcurrentContext()!=NULL) loader->GetcurrentContext()->clear();
